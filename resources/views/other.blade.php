...
<div style="padding-bottom: 40px">
    ...
    <div id="Mission">
        <p style="font-size: 1.25em;">{{trim($contentTable->where('title','=','MissionStatement')->where('type','=','University')->pluck('content'),'["]')}}
        </p>
    </div>
    @elseif(Auth::user()->type == 'COLLEGE_ADMIN')
        <div class="subtitle is-3">{{trim($joinedTable->where('username','=',Auth::user()->username)->pluck('abbrev'),'["]')}} Mission Statement
            <hr class="settings-hr">
        </div>
        <div id="Mission">
            <p style="font-size: 1.25em;">{{trim($contentTable->where('title','=','MissionStatement')->where('type','=',trim($joinedTable->where('username','=',Auth::user()->username)->pluck('abbrev'),'["]'))->pluck('content'),'["]')}}
            </p>
        </div>
    @endif
...
<div style="padding-bottom: 40px">
    ...
    <div id="Vision">
        <p style="font-size: 1.25em;">
            {{trim($contentTable->where('title','=','VisionStatement')->where('type','=','University')->pluck('content'),'["]')}}
        </p>
    </div>

    @elseif(Auth::user()->type == 'COLLEGE_ADMIN')
        <div class="subtitle is-3">{{trim($joinedTable->where('username','=',Auth::user()->username)->pluck('abbrev'),'["]')}} Vision Statement
            <hr class="settings-hr">
        </div>
        <div id="Vision">
            <p style="font-size: 1.25em;">
                {{trim($contentTable->where('title','=','VisionStatement')->where('type','=',trim($joinedTable->where('username','=',Auth::user()->username)->pluck('abbrev'),'["]'))->pluck('content'),'["]')}}
            </p>
        </div>
    @endif
...
<div style="padding-bottom: 40px">
    ...
    <div id="Principle">
        ...
        <ol style="font-size: 1.25em; padding-left: 50px">
            @foreach($principleTable->where('type','=','University') as $row)
    ...
    @elseif(Auth::user()->type == 'COLLEGE_ADMIN')
        <div class="subtitle is-3">{{trim($joinedTable->where('username','=',Auth::user()->username)->pluck('abbrev'),'["]')}} Program Outcome
            <hr class="settings-hr">
        </div>
        <div id="Principle">
            <p style="font-size: 1.25em;">
            <ol style="font-size: 1.25em; padding-left: 50px">
                @foreach($principleTable->where('type','=',trim($joinedTable->where('username','=',Auth::user()->username)->pluck('abbrev'),'["]')) as $row)
                    <li style="padding-bottom: 10px">{{$row->content}}</li>
                @endforeach
            </ol>
            </p>
...
<div id="missionStatementModal" class="modal" style="padding-top: 100px">
    ...
    <div class="modal-content">
        <div class="box">
            <form action="othercontent/@if(Auth::user()->type == 'SYSTEM_ADMIN'){{trim($contentTable->where('title','=','MissionStatement')->where('type','=','University')->pluck('id'),'["]')}} @elseif(Auth::user()->type=='COLLEGE_ADMIN'){{trim($contentTable->where('title','=','MissionStatement')->where('type','=',trim($joinedTable->where('username','=',Auth::user()->username)->pluck('abbrev'),'["]'))->pluck('id'),'["]')}}@endif" enctype="multipart/form-data" method="POST" id="editMissionStatement">
                @csrf
                {{ method_field('PATCH') }}
                <div class="subtitle is-3">Mission Statement
                    <hr class="settings-hr">
                </div>
                <h1>Content</h1>
                @if(Auth::user()->type == 'SYSTEM_ADMIN')
                    <textarea name="content" id="cntnt" cols="30" rows="10" style="width: 45em">{{trim($contentTable->where('title','=','MissionStatement')->where('type','=','University')->pluck('content'),'["]')}}</textarea>
                @elseif(Auth::user()->type=='COLLEGE_ADMIN')
                    <textarea name="content" id="cntnt" cols="30" rows="10" style="width: 45em">{{trim($contentTable->where('title','=','MissionStatement')->where('type','=',trim($joinedTable->where('username','=',Auth::user()->username)->pluck('abbrev'),'["]'))->pluck('content'),'["]')}}</textarea>
                @endif
...
<div id="visionStatementModal" class="modal" style="padding-top: 100px">
    ...
    <div class="modal-content">
        <div class="box">
            <form action="othercontent/@if(Auth::user()->type == 'SYSTEM_ADMIN'){{trim($contentTable->where('title','=','VisionStatement')->where('type','=','University')->pluck('id'),'["]')}} @elseif(Auth::user()->type=='COLLEGE_ADMIN'){{trim($contentTable->where('title','=','VisionStatement')->where('type','=',trim($joinedTable->where('username','=',Auth::user()->username)->pluck('abbrev'),'["]'))->pluck('id'),'["]')}}@endif" enctype="multipart/form-data" method="POST" id="editMissionStatement">
                @csrf
                {{ method_field('PATCH') }}
                <div class="subtitle is-3">Vision Statement
                    <hr class="settings-hr">
                </div>
                <h1>Content</h1>
                @if(Auth::user()->type == 'SYSTEM_ADMIN')
                    <textarea name="content" id="cntnt" cols="30" rows="10" style="width: 45em">{{trim($contentTable->where('title','=','VisionStatement')->where('type','=','University')->pluck('content'),'["]')}}</textarea>
                @elseif(Auth::user()->type=='COLLEGE_ADMIN')
                    <textarea name="content" id="cntnt" cols="30" rows="10" style="width: 45em">{{trim($contentTable->where('title','=','VisionStatement')->where('type','=',trim($joinedTable->where('username','=',Auth::user()->username)->pluck('abbrev'),'["]'))->pluck('content'),'["]')}}</textarea>
                @endif
                <button id="save">Save</button>
            </form>
            <button id="closeBtn" class="cls">close</button>
...
<div id="coreValuesModal" class="modal" style="padding-top: 100px">
    ...
    <div class="modal-content">
        <div class="box">
            <form action="othercontent/@if(Auth::user()->type == 'SYSTEM_ADMIN'){{trim($contentTable->where('title','=','CoreValues')->where('type','=','University')->pluck('id'),'["]')}}@elseif(Auth::user()->type == 'COLLEGE_ADMIN'){{trim($contentTable->where('title','=','Goals')->where('type','=',trim($joinedTable->where('username','=',Auth::user()->username)->pluck('abbrev'),'["]'))->pluck('id'),'["]')}}@endif" enctype="multipart/form-data" method="POST" id="editMissionStatement">
                @csrf
                {{ method_field('PATCH') }}
                @if(Auth::user()->type == 'SYSTEM_ADMIN')
                <div class="subtitle is-3">University Core Values
    ...
    <div class="modal-content">
        <div class="box">
            <table class="table is-fullwidth is-striped" id="principleTable">
                ...
                <tbody>
                @if(Auth::user()->type == 'SYSTEM_ADMIN')
                @foreach($principleTable->where('type','=','University') as $rows)
                    <tr>
                        <td>{{$rows->id}}</td>
                        <td>{{$rows->content}}</td>
                        <td>
                ...
                @endforeach
                    @elseif(Auth::user()->type=='COLLEGE_ADMIN')
                    @foreach($principleTable->where('type','=',trim($joinedTable->where('username','=',Auth::user()->username)->pluck('abbrev'),'["]')) as $rows)
                        <tr>
                            <td>{{$rows->id}}</td>
                            <td>{{$rows->content}}</td>
                            <td>
        ...
        <div class="box">
            <form action="othercontent/add" enctype="multipart/form-data" method="POST" id="editMissionStatement">
                ...
                <select name="Type" id="Type">
                    ...
                    @endforeach
                    @elseif(Auth::user()->type == 'COLLEGE_ADMIN')
                        <option value="{{trim($joinedTable->where('username','=',Auth::user()->username)->pluck('abbrev'),'["]')}}">{{trim($joinedTable->where('username','=',Auth::user()->username)->pluck('abbrev'),'["]')}}</option>
                    @endif
                </select>
                <h1>Content</h1>
