$(function() {
    $('#sb-accounts').addClass('is-active');
    $('#nb-accounts').addClass('is-active');
    let color = $('html').css('background-color');
    $('#header').css('border-bottom', '2px solid ' + color);
    $('#nb-accounts').css('border-left', '3px solid ' + color);
    $('.breadcrumb ul').append('<li class="is-active"><a><span class="icon is-medium"><i class="fas fa-users"></i></span>Logs</a></li>');
